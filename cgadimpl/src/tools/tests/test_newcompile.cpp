...
int main() {
    ...
    debug::print_tensor("Compiled Loss", loss.val());

    // ---------- Verification ----------
    // Compare the scalar result from the eager pass and the compiled pass
    float eager_val = p.val().to_cpu().data<float>()[0];
    float compiled_val = loss.val().to_cpu().data<float>()[0];
