...
y = cgadimpl.Tensor.ones([4, 4], device="cpu", requires_grad=True)
s = cgadimpl.Tensor.ones([4, 4], device="cpu", requires_grad=True)
r = cgadimpl.Tensor.ones([4, 4], device="cpu", requires_grad=True)
g = cgadimpl.Tensor.ones([4, 4], device="cpu", requires_grad=True)
...
m = cgadimpl.ops.attention(q,w,x,p)
# m = cgadimpl.ops.add(f,x)
