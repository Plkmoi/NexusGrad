...
void test_att( int H, int B, int S, int D)
{
    ...
    auto V = ag::Value(std::make_shared<ag::Node>(d_cpu.to(gpu_opts.device), ag::Op::Leaf, true, "X"));
        ...
        auto noutam = attention(realrms(z, 1.0), Q, Ka, V, H)+z;
    ...
    Tensor w_tensor = OwnTensor::Tensor::randn(Shape{{hidden_features, out_features}}, TensorOptions().with_device(Device::CUDA)) * scale;
    Tensor b_tensor = OwnTensor::Tensor::zeros(Shape{{1, hidden_features}}, TensorOptions().with_device(Device::CUDA));
    Tensor wa_tensor = OwnTensor::Tensor::randn(Shape{{hidden_features, out_features}}, TensorOptions().with_device(Device::CUDA)) * scale;
    Tensor ba_tensor = OwnTensor::Tensor::zeros(Shape{{1, hidden_features}}, TensorOptions().with_device(Device::CUDA));
    Tensor wc_tensor = OwnTensor::Tensor::randn(Shape{{out_features, hidden_features}}, TensorOptions().with_device(Device::CUDA)) * scale;
    ...
    auto outame = realrms(linear(swiglu(realrms(noutam, 1.0), W, b, Wa, ba),Wc, bc)+noutam, 1.0);
        auto neoutam = attention(realrms(outame, 1.0), Q, Ka, V, H)+outame;
    auto outam = realrms(linear(swiglu(realrms(neoutam, 1.0), W, b, Wa, ba),Wc, bc)+neoutam, 1.0);
