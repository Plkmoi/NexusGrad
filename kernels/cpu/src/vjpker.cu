...
void vjp_add_cuda(float* gA, float* gB, const float* gy,
    ...
    k_vjp_add_accum<<<blocks, 256>>>(gA, gB, gy, n);
}
...
void vjp_relu_cuda(float* gX, const float* gy, const float* X, int64_t n) {
    dim3 blocks( (unsigned int)((n + 255) / 256) );
    k_vjp_relu_accum<<<blocks, 256>>>(gX, gy, X, n);
}
...
void vjp_tanh_cuda(float* gX, const float* gy, const float* X, int64_t n) {
    dim3 blocks( (unsigned int)((n + 255) / 256) );
    k_vjp_tanh_accum<<<blocks, 256>>>(gX, gy, X, n);
}