...
std::vector<int> all_tokens(all_tokens_u32.begin(), std::next(std::begin(all_tokens_u32), 2000));
std::vector<uint32_t> all_tokensi_u32(all_tokens_u32.begin(), std::next(std::begin(all_tokens_u32), 2000));
...
vocab_size += 1;
    ...
    std::copy(all_tokens.begin(), all_tokens.end(), std::ostream_iterator<int>(std::cout, " "));
    std::cout << std::endl;

Tensor embedding_table =
    flow::make_embedding_table(vocab_size, d_model);

    Tensor X_cpu_new = flow::embed_tokens(embedding_table, all_tokens);
    auto m = OwnTensor::reduce_max(X_cpu_new, {-1});
    ag::debug::print_tensor("What I embedded", m);

    std::string decoded = tok.decode(all_tokensi_u32);
