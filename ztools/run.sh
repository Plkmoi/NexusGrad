...
# cp "$KERNELS_BUILD/cpu/libflowkernels_cpu.so" "$CORE_BUILD/"
# cp "$KERNELS_BUILD/gpu/libflowkernels_cuda.so" "$CORE_BUILD/"
...
cmake -S "$KERNELS_DIR" -B "$KERNELS_BUILD" \
  -DCMAKE_BUILD_TYPE="$BUILD_TYPE" \
  -DARCH_INCLUDE_DIR="$ARCH_DIR/include"
...
echo "== Copying kernel plugins to test directory"
cp "$KERNELS_BUILD/cpu/libflowkernels_cpu.so" "$ARCH_BUILD/"
cp "$KERNELS_BUILD/gpu/libflowkernels_cuda.so" "$ARCH_BUILD/"
